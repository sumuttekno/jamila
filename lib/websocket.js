...
this.Jamila = (function() {
  function Jamila(_at_url, _at_use_websockets) {
    this.url = _at_url;
  ...
  Jamila.prototype.connect = function() {
    this.state = 'connecting';
    if (!(this.supports_websockets() && this.use_websockets)) {
      this._conn = new Jamila.HttpConnection(this.url, this);
    } else {
      this._conn = new Jamila.WebSocketConnection(this.url, this);
    }
    return this._conn.new_message = this.new_message;
  };

  Jamila.prototype.disconnect = function() {
    if (this._conn) {
      this._conn.close();
  ...
  Jamila.prototype.reconnect = function() {
  ...
  Jamila.prototype.new_message = function(data) {
    var event, socket_message, _i, _len, _ref, _results;
    _results = [];
    for (_i = 0, _len = data.length; _i < _len; _i++) {
      socket_message = data[_i];
      event = new Jamila.Event(socket_message);
      if (event.is_result()) {
  ...
  Jamila.prototype.connection_established = function(data) {
  ...
  Jamila.prototype.bind = function(event_name, callback) {
    var _base;
  ...
  Jamila.prototype.unbind = function(event_name) {
    return delete this.callbacks[event_name];
  };

  Jamila.prototype.trigger = function(event_name, data, success_callback, failure_callback) {
    var event, _ref;
    event = new Jamila.Event([event_name, data, (_ref = this._conn) != null ? _ref.connection_id : void 0], success_callback, failure_callback);
    return this.trigger_event(event);
  };

  Jamila.prototype.trigger_event = function(event) {
    var _base, _name;
  ...
  Jamila.prototype.dispatch = function(event) {
  ...
  Jamila.prototype.subscribe = function(channel_name, success_callback, failure_callback) {
    var channel;
    if (this.channels[channel_name] == null) {
      channel = new Jamila.Channel(channel_name, this, false, success_callback, failure_callback);
  ...
  Jamila.prototype.subscribe_private = function(channel_name, success_callback, failure_callback) {
    var channel;
    if (this.channels[channel_name] == null) {
      channel = new Jamila.Channel(channel_name, this, true, success_callback, failure_callback);
      this.channels[channel_name] = channel;
      return channel;
    } else {
      return this.channels[channel_name];
  ...
  Jamila.prototype.unsubscribe = function(channel_name) {
    if (this.channels[channel_name] == null) {
      return;
    }
  ...
  Jamila.prototype.dispatch_channel = function(event) {
  ...
  Jamila.prototype.supports_websockets = function() {
    return typeof WebSocket === "function" || typeof WebSocket === "object";
  };

  Jamila.prototype.pong = function() {
    var pong, _ref;
    pong = new Jamila.Event(['websocket_rails.pong', {}, (_ref = this._conn) != null ? _ref.connection_id : void 0]);
    return this._conn.trigger(pong);
  };

  Jamila.prototype.connection_stale = function() {
    return this.state !== 'connected';
  };

  Jamila.prototype.reconnect_channels = function() {
